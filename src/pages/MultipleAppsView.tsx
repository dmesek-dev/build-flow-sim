...
const MultipleAppsView: React.FC = () => {
  ...
  const startPipeline = () => {
    ...
    const idList = pharmacyIds.split(',').map(id => id.trim()).filter(id => id);
    
    if (idList.length === 0) {
      setBuildSteps([
        { name: 'Error: No valid Pharmacy IDs provided.', status: 'failed' }
      ]);
      return;
    }

    setBuildSteps([
      { name: `Starting pipeline for ${idList.length} Pharmacy IDs: ${pharmacyIds}...`, status: 'pending' }
    ]);
  ...
  const processBatch = (idList: string[]) => {
    setBuildSteps(prev => [
      ...prev, 
      { name: `Starting batch process for ${idList.length} pharmacy IDs...`, status: 'pending' }
    ]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <h1 className="text-3xl font-bold tracking-tight">Multiple Apps Pipeline</h1>
          ...
          <Card>
            ...
            <CardContent>
              <PipelineSelector 
                ...
                labelOverride="Pharmacy IDs"
                placeholder="Enter comma-separated pharmacy IDs"
              />
              <PipelineVisualizer 
...
export default MultipleAppsView;
